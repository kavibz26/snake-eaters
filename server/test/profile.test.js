...
test('single-player result adapter: uses the game\'s own totals; play time is game time (ticks), so pauses do not count', () => {
  ...
  assert.deepEqual(r, { key: 'run-9', mode: 'single', victory: true, survived: true, score: 260, length: 18, kills: 3, food: 11, powerups: undefined, mega: undefined, events: [], playSeconds: 60 });
...
test('multiplayer result adapter: only the server\'s figures, only for a player who is in the results', () => {
  ...
  assert.deepEqual(fromMultiplayer(over, 'p1', 'm1', 75), { key: 'm1', mode: 'multiplayer', victory: true, survived: true, score: 330, length: 14, kills: 2, food: 13, powerups: undefined, mega: undefined, events: [], playSeconds: 75 });
